...
NS_LOG_COMPONENT_DEFINE("Ipv4TorusRouting");

NS_OBJECT_ENSURE_REGISTERED(Ipv4TorusRouting);

TypeId Ipv4TorusRouting::GetTypeId() {
  static TypeId tid = TypeId("ns3::Ipv4TorusRouting")
                          .SetParent<Ipv4RoutingProtocol>()
                          .SetGroupName("Internet")
                          .AddConstructor<Ipv4TorusRouting>();
  return tid;
}

Ipv4TorusRouting::Ipv4TorusRouting() : m_ipv4(nullptr) {
  NS_LOG_FUNCTION(this);
}

void Ipv4TorusRouting::AddNetworkRouteTo(Ipv4Address network,
                                         Ipv4Mask networkMask,
                                         Ipv4Address nextHop,
                                         uint32_t interface, uint32_t metric) {
  NS_LOG_FUNCTION(this << network << " " << networkMask << " " << nextHop << " "
                       << interface << " " << metric);

  Ipv4RoutingTableEntry route = Ipv4RoutingTableEntry::CreateNetworkRouteTo(
      network, networkMask, nextHop, interface);
...
void Ipv4TorusRouting::AddHostRouteTo(Ipv4Address dest, Ipv4Address nextHop,
                                      uint32_t interface, uint32_t metric) {
...
void Ipv4TorusRouting::AddHostRouteTo(Ipv4Address dest, uint32_t interface,
                                      uint32_t metric) {
  NS_LOG_FUNCTION(this << dest << " " << interface << " " << metric);
  AddNetworkRouteTo(dest, Ipv4Mask::GetOnes(), interface, metric);
}

void Ipv4TorusRouting::SetDefaultRoute(Ipv4Address nextHop, uint32_t interface,
                                       uint32_t metric) {
...
void Ipv4TorusRouting::AddMulticastRoute(
  ...
  *route = Ipv4MulticastRoutingTableEntry::CreateMulticastRoute(
...
void Ipv4TorusRouting::SetDefaultMulticastRoute(uint32_t outputInterface) {
...
uint32_t Ipv4TorusRouting::GetNMulticastRoutes() const {
  NS_LOG_FUNCTION(this);
  return m_multicastRoutes.size();
}

Ipv4MulticastRoutingTableEntry
Ipv4TorusRouting::GetMulticastRoute(uint32_t index) const {
  NS_LOG_FUNCTION(this << index);
  NS_ASSERT_MSG(index < m_multicastRoutes.size(),
                "Ipv4TorusRouting::GetMulticastRoute ():  Index out of range");
...
bool Ipv4TorusRouting::RemoveMulticastRoute(Ipv4Address origin,
                                            Ipv4Address group,
...
void Ipv4TorusRouting::RemoveMulticastRoute(uint32_t index) {
  NS_LOG_FUNCTION(this << index);
  uint32_t tmp = 0;
  for (auto i = m_multicastRoutes.begin(); i != m_multicastRoutes.end(); i++) {
    if (tmp == index) {
...
bool Ipv4TorusRouting::LookupRoute(const Ipv4RoutingTableEntry &route,
                                   uint32_t metric) {
...
Ptr<Ipv4Route> Ipv4TorusRouting::ComputeRoute(Ipv4Address dest,
  ...
  if (dest.IsLocalMulticast()) {
    ...
    rtentry->SetSource(
        m_ipv4->GetAddress(m_ipv4->GetInterfaceForDevice(oif), 0).GetLocal());
    return rtentry;
  }
  ...
  for (auto i = m_networkRoutes.begin(); i != m_networkRoutes.end(); i++) {
    ...
    NS_LOG_LOGIC("Searching for route to "
                 << dest << ", checking against route to " << entry << "/"
                 << masklen);
...
uint32_t Ipv4TorusRouting::GetNRoutes() const {
  NS_LOG_FUNCTION(this);
...
Ipv4RoutingTableEntry Ipv4TorusRouting::GetDefaultRoute() {
  NS_LOG_FUNCTION(this);
  // Basically a repeat of ComputeRoute, retained for backward compatibility
...
Ipv4RoutingTableEntry Ipv4TorusRouting::GetRoute(uint32_t index) const {
  NS_LOG_FUNCTION(this << index);
  uint32_t tmp = 0;
  for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++) {
    if (tmp == index) {
...
uint32_t Ipv4TorusRouting::GetMetric(uint32_t index) const {
  NS_LOG_FUNCTION(this << index);
  uint32_t tmp = 0;
  for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++) {
    if (tmp == index) {
...
void Ipv4TorusRouting::RemoveRoute(uint32_t index) {
  NS_LOG_FUNCTION(this << index);
  uint32_t tmp = 0;
  for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++) {
    if (tmp == index) {
...
Ptr<Ipv4Route> Ipv4TorusRouting::RouteOutput(Ptr<Packet> p,
...
bool Ipv4TorusRouting::RouteInput(
    Ptr<const Packet> p, const Ipv4Header &ipHeader, Ptr<const NetDevice> idev,
  ...
  NS_LOG_FUNCTION(this << p << ipHeader << ipHeader.GetSource()
                       << ipHeader.GetDestination() << idev << &ucb << &mcb
                       << &lcb << &ecb);
  ...
  if (ipHeader.GetDestination().IsMulticast()) {
    ...
    Ptr<Ipv4MulticastRoute> mrtentry =
        LookupStatic(ipHeader.GetSource(), ipHeader.GetDestination(),
...
Ipv4TorusRouting::~Ipv4TorusRouting() { NS_LOG_FUNCTION(this); }

void Ipv4TorusRouting::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void Ipv4TorusRouting::NotifyRemoveAddress(uint32_t interface,
                                           Ipv4InterfaceAddress address) {
  NS_LOG_FUNCTION(this << interface << " " << address.GetLocal());
  if (!m_ipv4->IsUp(interface)) {
    return;
...
void Ipv4TorusRouting::SetIpv4(Ptr<Ipv4> ipv4) {
...
void Ipv4TorusRouting::PrintRoutingTable(Ptr<OutputStreamWrapper> stream,
  ...
  *os << "Node: " << m_ipv4->GetObject<Node>()->GetId()
      ...
      << ", Ipv4TorusRouting table" << std::endl;
