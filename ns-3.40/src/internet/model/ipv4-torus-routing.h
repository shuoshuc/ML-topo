...
#ifndef IPV4_TORUS_ROUTING_H
#define IPV4_TORUS_ROUTING_H
 ...
 * \ingroup ipv4Routing
 *
 * \brief Torus routing protocol for IP version 4 stacks.
 *
 ...
 * protocol.
 *
 * The Ipv4TorusRouting class inherits from the abstract base class
 ...
 */
class Ipv4TorusRouting : public Ipv4RoutingProtocol {
public:
  /**
   ...
   * destination.
   * \param metric Metric of route in case of multiple routes to same
   * destination
   *
   * \see Ipv4Address
   */
   ...
   *
   * This method tells the routing system what to do in the case where a
   * specific route to a destination is not found.  The system forwards packets
   * to the specified node in the hope that it knows better how to route the
   * packet.
   *
   * If the default route is set, it is returned as the selected route from
   * ComputeRoute irrespective of destination address if no specific route is
   * found.
   *
  ...
  /**
   * \brief Get the default route with lowest metric from the static routing
   * table.
   *
   * \return If the default route is set, a pointer to that
   * Ipv4RoutingTableEntry is returned, otherwise an empty routing table entry
   * is returned. If multiple default routes exist, the one with lowest metric
   * is returned.
   *
   ...
   * \return If route is set, a pointer to that Ipv4RoutingTableEntry is
   * returned, otherwise a zero pointer is returned.
   *
   * \see Ipv4RoutingTableEntry
   * \see Ipv4TorusRouting::RemoveRoute
   */
   ...
   * \return If route is set, the metric is returned. If not, an infinity metric
   ...
   *
   * \see Ipv4RoutingTableEntry
   * \see Ipv4TorusRouting::GetRoute
   * \see Ipv4TorusRouting::AddRoute
   */
   ...
   * \param outputInterfaces A vector of network interface indexes used to
   * specify how to send packets to the destination(s).
   *
   * \see Ipv4Address
   */
   ...
   * retrieve.
   * \return If route \e i is set, a pointer to that
   * Ipv4MulticastRoutingTableEntry is returned, otherwise a zero pointer is
   * returned.
   *
   ...
   *
   * \see Ipv4MulticastRoutingTableEntry
   * \see Ipv4TorusRouting::AddMulticastRoute
   */
